...
const getScriptPath = () => {
  if (app.isPackaged) {
    ...
    return path.join(__dirname$1, "../../trafikverket_status_module.exe");
  }
};
const getPythonScriptPath = () => {
  if (app.isPackaged) {
    ...
    return path.join(__dirname$1, "trafikverket_status_module.py");
  }
};
const isValidResult = (raw) => {
  return typeof raw === "object" && typeof raw.id === "number" && typeof raw.une_id === "string" && typeof raw.une_id_raw === "string" && typeof raw.une === "string" && typeof raw.driftsomr === "string" && typeof raw.bandel === "number" && typeof raw.coverage_pct === "number" && typeof raw.tested_length_km === "number" && typeof raw.total_length_km === "number" && typeof raw.km_from === "number" && typeof raw.km_to === "number" && typeof raw.status === "string" && (raw.planned_date === null || typeof raw.planned_date === "string") && (raw.tested_date === null || typeof raw.tested_date === "string") && (raw.last_previous_test === null || typeof raw.last_previous_test === "string") && (raw.next_test_date === null || typeof raw.next_test_date === "string") && (raw.days_until === null || typeof raw.days_until === "number") && (raw.deadline === null || typeof raw.deadline === "string") && typeof raw.deadline_status === "string" && Array.isArray(raw.gaps) && raw.gaps.every(
...
function runStatusModuleAll({
  ...
  console.log("Using script path:", scriptPath);
  const cmd = `${useExe ? "" : "python "}"${scriptPath}" --all --tested "${testedPath}" --untested "${untestedPath}" --testplan "${planPath}"`;
  const executeWithRetry = async (retries = 3, delay = 1e3) => {
